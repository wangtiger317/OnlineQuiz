...
//                $this->add_edit();
                ...
                $this->load->view('include/script');
                $this->load->view('register', $send_data);
            }
        } else {
...
//                $this->add_edit();
    ...
    public function profile($id = '')
    {
        ...
        $data['total_teacher'] = count($this->User_model->get_members('teacher'));
        $data['total_student'] = count($this->User_model->get_members('student'));
        $data['total_subjects'] = count($this->Subject_model->get_all_subjects());
        if($this->session->userdata('user_details')[0]->user_type === 'admin') {
            $data['total_absence'] = count($this->Absense_model->get_data());
            $data['total_quiz'] = count($this->Quiz_model->get_all_quiz());
        } else if($this->session->userdata('user_details')[0]->user_type == 'Teacher'){
            $subject = $this->session->userdata('user_details')[0]->subject;
            $data['total_absence'] = count($this->Absense_model->get_data_subject($subject));
            $data['total_quiz'] = count($this->Quiz_model->get_quiz($subject));
        }
    ...
    {
        is_login();
        $all_users = $this->User_model->get_members('teacher');
        $data['all_users'] = $all_users;
//        if (CheckPermission("user", "own_read")) {
            $this->load->view('include/header');
            $this->load->view('teachertable', $data);
            $this->load->view('include/footer');
//        } else {
...
//        }
    ...
    {
        is_login();
        $all_users = $this->User_model->get_members('student');
        $data['all_users'] = $all_users;
//        if (CheckPermission("user", "own_read")) {
            $this->load->view('include/header');
...
//        }
    ...
    public function AbsenseTable()
    {
        is_login();
        $students = $this->Absense_model->get_data();
        $subjects = $this->Subject_model->get_all_subjects();
        $data['subjects'] = $subjects;
        $data['students'] = $students;
//        if (CheckPermission("user", "own_read")) {
        $this->load->view('include/header');
        $this->load->view('absensetable', $data);
        $this->load->view('include/footer');
//        } else {
//            $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
//            redirect(base_url() . 'user/profile', 'refresh');
//        }
    ...
    public function TeacherAbsenseTable()
    {
        is_login();
        $students = $this->Absense_model->get_data();
        $subjects = $this->Subject_model->get_all_subjects();
...
//        if (CheckPermission("user", "own_read")) {
        $this->load->view('include/header');
        $this->load->view('teacherabsensetable', $data);
        $this->load->view('include/footer');
//        } else {
//            $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
//            redirect(base_url() . 'user/profile', 'refresh');
...
//        }
    ...
    public function StudentAbsenseTable()
    {
        is_login();
        $student_id = $this->session->userdata('user_details')[0]->users_id;
        $students = $this->Absense_model->get_student($student_id);
        $subjects = $this->Subject_model->get_all_subjects();
        $data['subjects'] = $subjects;
        $data['students'] = $students;
//        if (CheckPermission("user", "own_read")) {
        $this->load->view('include/header');
        $this->load->view('studentabsensetable', $data);
        $this->load->view('include/footer');
//        } else {
//            $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
//            redirect(base_url() . 'user/profile', 'refresh');
//        }
    ...
    public function removerow(){
        $users_id = $_GET['id'];
        $this->User_model->delete($users_id);
        echo true;
    }
    ...
    public function updaterow(){
        ...
        for ($i = 1; $i < count($th_names); $i++){
            ...
            $this->User_model->updateRow($table, 'users_id', $users_id, $data);
        }
    }
    ...
    public function insertRow(){
        ...
        $data['password'] = password_hash($data['name'], PASSWORD_DEFAULT);
        $this->User_model->insertRow($table, $data);
        echo $data['name'];

    }

    public function addabsencedata(){
        ...
        $data['day'] = $_GET['day'];
        $data['date'] = $_GET['date'];
        $data['attendance_time'] = $_GET['attendance_time'];
        $data['score'] = $_GET['score'];
        $subject_id = $this->Subject_model->get_subject_id($_GET['subject']);
        $data['subject_id'] = $subject_id;
        $result = $this->Absense_model->insertRow('absence', $data);
        echo $result;
    }
